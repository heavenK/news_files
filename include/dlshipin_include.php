...
class SearchView
{
	...
	$searchtype='',$starttime=0,$upagesize=20,$kwtype=1,$mid=0)
	{
		...
		$this->CountRecord();
		$tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/plus/list_dlshipin_new.htm";
	...
	$imgwidth=120,$imgheight=90,$achanneltype="all",$orderby="default",$innertext="",$tablewidth="100")
	{
		...
		if($this->ChannelType< 0 ||$this->ChannelTypeid< 0){
			...
			else if($orderby=="apubdate")
			{
				$ordersql=" order by arc.pubdate asc";
			}
			else if($orderby=="id")
			{
