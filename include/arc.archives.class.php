	...
	function ParseDMFields($pageNo,$ismake=1)
	{
		...
		if(is_array($this->dtp->CTags))
		{
			foreach($this->dtp->CTags as $i=>$ctag)
			{
				...
				//edangel
				else if($ctag->GetName()=='edprenext')
                {
                    $this->dtp->Assign($i,$this->GetPreNext2($ctag->GetAtt('get')));
                }
				//edangel end
    ...
    function GetPreNext2($gtype='')
    {
        ...
        if(count($this->PreNext)<2)
        {
            ...
            if(is_array($preRow))
            {
                $mlink = GetFileUrl($preRow['id'],$preRow['typeid'],$preRow['senddate'],$preRow['title'],$preRow['ismake'],$preRow['arcrank'],
                $preRow['namerule'],$preRow['typedir'],$preRow['money'],$preRow['filename'],$preRow['moresite'],$preRow['siteurl'],$preRow['sitepath']);
                $this->PreNext['pre'] = "<a href='$mlink'><div style=\" width:36px; height:36px; float:left; background:url(/images/second_pub/second_pub_four/prev.jpg) no-repeat; position:absolute; top:80px; left:10px;\"></div></a> ";
                $this->PreNext['preimg'] = "<a href='$mlink'><img src=\"{$preRow['litpic']}\" alt=\"{$preRow['title']}\"/></a> "; 
            }
            else
            {
                $this->PreNext['pre'] = "<div style=\" width:36px; height:36px; float:left; background:url(/images/second_pub/second_pub_four/prev.jpg) no-repeat; position:absolute; top:80px; left:10px;\"></div>";
                $this->PreNext['preimg'] ="<img src=\"/templets/default/images/nophoto.jpg\" alt=\"对不起，没有上一图集了！\"/>";
            }
            if(is_array($nextRow))
            {
                $mlink = GetFileUrl($nextRow['id'],$nextRow['typeid'],$nextRow['senddate'],$nextRow['title'],$nextRow['ismake'],$nextRow['arcrank'],
                $nextRow['namerule'],$nextRow['typedir'],$nextRow['money'],$nextRow['filename'],$nextRow['moresite'],$nextRow['siteurl'],$nextRow['sitepath']);
                $this->PreNext['next'] = "<a href='$mlink'><div style=\" width:36px; height:36px; float:left; background:url(/images/second_pub/second_pub_four/next.jpg) no-repeat; position:absolute; top:80px; left:122px;\"></div></a> ";
                $this->PreNext['nextimg'] = "<a href='$mlink'><img src=\"{$nextRow['litpic']}\" alt=\"{$nextRow['title']}\"/></a> ";
            }
            else
            {
                $this->PreNext['next'] = "<div style=\" width:36px; height:36px; float:left; background:url(/images/second_pub/second_pub_four/next.jpg) no-repeat; position:absolute; top:80px; left:122px;\"></div>";
                $this->PreNext['nextimg'] ="<a href='javascript:void(0)' alt=\"\"><img src=\"/templets/default/images/nophoto.jpg\" alt=\"对不起，没有下一图集了！\"/></a>";
            }
        }
        if($gtype=='pre')
