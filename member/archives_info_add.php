...
if(empty($dopost))
{
	...
	include(DEDEMEMBER."/templets/archives_info_add.htm");
	exit();
}
/*------------------------------
...
else if($dopost=='save')
{
	...
	$insql = "INSERT INTO `#@__infop` values('','$arcID','$uname','$nickname')";
	if(!$dsql->ExecuteNoneQuery($insql))
	{
		ShowMsg("把数据保存到数据库用户关联表时出错，请联系管理员！","javascript:;");
		exit();
	}
...
//	$cfg_ml->RecordFeeds('add',$title,$description,$arcID);
	...
	$msg = "
	　　请选择你的后续操作：
		<a href='archives_info_add.php?channelid=$channelid'><u>继续发布内容</u></a>
		&nbsp;&nbsp;
		<a href='$artUrl' target='_blank'><u>查看内容</u></a>
		&nbsp;&nbsp;
		<a href='archives_info_edit.php?channelid=$channelid&aid=$arcID'><u>更改内容</u></a>
		&nbsp;&nbsp;
		<a href='content_info_list.php?channelid={$channelid}'><u>已发布内容管理</u></a>
		";
	$wintitle = "成功发布内容！";
