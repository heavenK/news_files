...
if(empty($dopost))
{
  ...
  $arcQuery = "Select ch.*,arc.* From `#@__arctiny` arc
	...
	$isUser = $dsql->GetOne("Select * From `#@__infop` where aid='$aid'; ");
	if($isUser['username']!=$uname)
	{
		ShowMsg("对不起，你没权限操作此文档！","-1");
		exit();
	}
	$addRow['id'] = $addRow['aid'];
	include(DEDEMEMBER."/templets/archives_info_edit.htm");
	exit();
}
...
else if($dopost=='save')
{
	...
	//返回成功信息
	$msg = "　　请选择你的后续操作：
		<a href='archives_info_add.php?cid=$typeid'><u>发布新内容</u></a>
		&nbsp;&nbsp;
		<a href='archives_info_edit.php?channelid=$channelid&aid=".$aid."'><u>查看更改</u></a>
		&nbsp;&nbsp;
		<a href='$artUrl' target='_blank'><u>查看内容</u></a>
		&nbsp;&nbsp;
		<a href='content_info_list.php?channelid=$channelid'><u>管理内容</u></a>
		";
	$wintitle = "成功更改内容！";
