...
if($cfg_feedback_forbid=='Y') exit('系统评论功能升级，暂时关闭！');
...
//	}
	...
	@session_start();
	$username = $_SESSION["Zend_Auth"]['storage']->nickname;
	
	if ($username == '') {
		ShowMsg('请登录！','http://passport.we54.com');
		exit();
	}
	if(empty($face))
	{
	...
	elseif ($comtype == 'reply')
	{
		...
		$quotemsg = $row['quotemsg'];
		//{title}'.$row['username'].' 的原帖：{/title}.$row['username'].' 的原帖：{/title}{content}'.$row['msg'].'{/content}';
		$oldmsg	=	'{quote}'.$quotemsg.'{title}'.$row['username'].' 的原帖：{/title}{content}'.$row['msg'].'{/content}{/quote}';
		//$quotemsg = '{quote}'.$quotemsg;
		//$msg	=	$oldmsg.'{content}'.$msg.'{/content}';//{title}'.$row['username'].'{/title}
		
		//$msg = $quotemsg.$msg;
		$msg = HtmlReplace($msg,2);
		$inquery = "INSERT INTO `#@__feedback`(`aid`,`typeid`,`username`,`arctitle`,`ip`,`ischeck`,`dtime`,`mid`,`bad`,`good`,`ftype`,`face`,`msg`,`quotemsg`)
				VALUES ('$aid','$typeid','$username','$arctitle','$ip','$ischeck','$dtime','{$cfg_ml->M_ID}','0','0','$feedbacktype','$face','$msg','$oldmsg')";
