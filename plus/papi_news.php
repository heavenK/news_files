...
if($row){
	...
	$dsql->SetQuery("Select ar.id as newsID, ar.title as newsTitle,ar.source as newsSource,ar.pubdate as newsDate, ad.body as newsContent, ad.shipin as newsFlv From `#@__archives` ar LEFT JOIN `".$row['addtable']."` ad ON ar.id = ad.aid WHERE ar.id=".$newsID);
	$dsql->Execute();
	$res = $dsql->GetArray();
...
//$content = $res['newsFlv'];
	preg_match($reg_src,$content,$match);
	
	if($test) var_dump($match);
	
	$res['newsContent'] = preg_replace($reg_obj,"",$content);
	$res['newsContent'] = preg_replace($reg_embed,"",$res['newsContent']);
	$res['newsContent'] = str_replace("#p#∏±±ÍÃ‚#e#","",$res['newsContent']);
